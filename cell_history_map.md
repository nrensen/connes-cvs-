...
3. **Dirichlet Boundary Extinction:**
   Audit the scaling of the extinction products $P_T(N) \equiv \frac{|T_{v_N}(0)|}{\sqrt{2}} N^{3/2}$ and $P_\alpha(N) \equiv |\alpha_N| \sqrt{N}$ across discrete dimensions $N \in \{32, 48, 64, 96, 128, 192\}$ to verify that boundary contact decay quenches the $N^{3/2}$ modal growth by over 20 orders of magnitude.
4. **Core Cross-Coupling Stability (Proposition 2 in [`cell108.md`](file:///c:/data/github/connes-cvs-/cell108.md)):**
   Prove that $\|B_M^T u_N^{(P)}\|_2 \le \|H_N\|_{\mathrm{op}} \|u_N^{(P)}\|_2 \le 59.0$ is unconditionally bounded for all $N \ge 64$, and audit its numerical convergence to $\|B_M^T u_\infty^{(P)}\|_2$ across $N$ for fixed cutoffs $M \in \{24, 32, 48, 64\}$.
5. **The Ground-State Regularity Bridge (Theorem 2 in [`cell108.md`](file:///c:/data/github/connes-cvs-/cell108.md)):**
   Audit the stability of the total high-sector resolvent upper bound $\frac{1}{\delta_M} (\|\xi_N^{(Q)}\|_2 + \|B_M^T u_N^{(P)}\|_2)$ across $N$, verifying uniform Sobolev control $\sup_N \mathcal{K}_2(N) < \infty$.

### Status
**Established analytically / Authored computational verification.** Companion research note [`cell108.md`](file:///c:/data/github/connes-cvs-/cell108.md) completed; companion high-precision script [`cell108.py`](file:///c:/data/github/connes-cvs-/cell108.py) ready for execution on compute nodes.

---

# Updated major historical arc (Cells 0–108)
...
Cells 90–97 (Phase VII)
    ...
    ↓
Cells 98–108 (Phase VIII)
    Feshbach/Schur decoupling, exact eigenvector complementarity w_M = -(C-E)v^(Q), two-sided tail sandwich Delta E ~ ||v^(Q)||^2, commutator algebra, and the boundary-controlled regularity bridge
```
...
* **Excited Bound-State Sector (Paper NR3 Scope):** Discrete Sturm–Liouville nodal ladders, universal bound-state transmission zeros at Riemann zeros $\gamma_1 \dots \gamma_5$, multi-$c$ gap universality, and sharp localization transition separating confined bound states from delocalized scattering continuum.
