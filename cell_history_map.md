...
Cell 139 (Gate 1 Functional Spectral Tradeoff Inequality & Cross-Gram Geometry)
    Formulation of the functional spectral tradeoff inequality Delta K >= F(Delta W) on B_{11}^perp via the full cross-Gram transition matrix O_{j, k} = |<x_j, y_k>|^2; spectral distribution bounds and quantitative incompatibility between kinetic and potential sectors
```
