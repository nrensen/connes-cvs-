...
By the Kato–Lax–Milgram–Nelson (KLMN) theorem, there exists a unique self-adjoint Hamiltonian $H_{\mathrm{comp}}$ on $L^2([0, L])$ whose form domain is $\mathcal{Q}(H_{\mathrm{comp}}) = H^{\log}([0, L])$.
...
3. **Closedness:** Since $\mathcal{D} - \mathcal{W}$ is a bounded perturbation with $\|\mathcal{D} - \mathcal{W}\|_{\mathrm{op}} \le \|\mathcal{D}\|_{\mathrm{op}} + \|\mathcal{W}\|_{\mathrm{op}} \le 3 W(L) < \infty$, the perturbation is form-bounded with relative bound $\alpha = 0 < 1$. By the KLMN theorem (Reed & Simon Vol II, Theorem X.17), $\mathcal{E}$ is closed on $\mathcal{Q}(\mathcal{A}) = H^{\log}([0, L])$ and uniquely defines a self-adjoint Friedrichs operator $H_{\mathrm{comp}}$. $\blacksquare$
...
In Paper NR2 (Proposition 8.33) and Cell 90–97, the spectral projectors $P_{11}^{(N)} = \sum_{k=0}^{10} u_k^{(N)} (u_k^{(N)})^T$ were proven to satisfy Cauchy convergence in operator norm:
...
## 5. The Continuum Variational Lower Bound
...
### 5.1 The Four Interlocking Structural Barriers
The constrained infimum cannot drop deeper than $\approx -0.4870$ due to four interlocking mathematical mechanisms:
...
2. **Ground-State Orthogonality Suppression:**
   ...
   $$\sum_{m=0}^\infty |c_m|^2 \Omega(m) \ge |c_0|^2 h_+(0) + 0.1567 (1 - |c_0|^2) \ge -0.38 + 0.15 = -0.23.$$
...
4. **Phase Interference and Potential Dilution:**
   Because $T \perp u_0, \dots, u_{10}$, $T(t)$ is forced to oscillate with high frequency (peak mode $m^* = 26$). By Riemann–Lebesgue oscillatory cancellation against the step functions $\mathbf{1}_{[\log q, L]}$, the wavepacket cannot harvest more than $37.31\%$ of the well depth:
   $$\mathcal{W}[T] \le 0.38 W(L) \approx 3.78.$$

### 5.2 The Coercive Balance
Combining these terms yields the empirical continuum balance:
$$\mathcal{E}[T] = \mathcal{A}[T] - \mathcal{W}[T] + \mathcal{D}[T] \approx 1.174 - 3.710 + 2.049 = -0.4870.$$
The distance above $-1/2$ is:
$$\boxed{\Delta_{\mathrm{margin}} \equiv \mu_0^{(\infty)} - \left(-\frac{1}{2}\right) \approx +0.0130 > 0.}$$
Because $R_{\mathrm{arch}} + R_{\mathrm{pole}} \approx 0.440 + 0.212 = 0.652 > 0.487$, this deficit is decisively restored in the full operator to net positive energy $R_{\mathrm{net}} \approx +0.165 > 0$.
