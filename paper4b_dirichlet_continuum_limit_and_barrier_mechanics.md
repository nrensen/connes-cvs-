...
### Proposition 8.4 (Exact Small-Denominator Cancellation and Collective Spectral Reorganization)
*Using the exact algebraic small-denominator cancellation theorem from Paper 4 (Theorem 7.2), the apparent singularity $(E_k - \lambda)^{-1}$ in the spectral expansion of $(Q_{\mathrm{even}} - \lambda I)^\dagger \mathbf{w}$ cancels identically against the odd arithmetic resolvent difference in the numerator. For each even state $k$, the summand reduces to:*

$$\frac{(e^T u^{(k)}) (u^{(k)T} \mathbf{w})}{E_k - \lambda} \equiv - [D_0^{(k)}]^2 \cdot \langle p, (Q_{\mathrm{odd}} - E_k I)^{-1} (Q_{\mathrm{odd}} - \lambda I)^{-1} p \rangle.$$

*Empirical Findings on Bound States and Collective Reorganization (`cell55.py`).*
1. **Algebraic Cancellation:** High-precision evaluation confirms that the factor $(E_k - \lambda)$ cancels identically to machine precision ($10^{-32}$ on matrix operators and $8.8 \times 10^{-12}$ on individual modes), proving that the resolvent coupling is non-singular.
2. **Refutation of Simple Scattering Dominance:** While the boundary value $D_0^{(0)} = e^T u^{(0)} \sim 10^{-20}$ vanishes precipitously for the tunneling ground state $k = 0$, for excited bound states ($k \ge 1$) the boundary amplitude $D_0^{(k)} = e^T u^{(k)}$ is $\mathcal{O}(1)$. Because the numerator $D_0^{(k)} (w^T u^{(k)})$ and eigenvalue $E_k$ scale in exact proportionality, the cancelled terms are finite positive numbers ($T_1 \approx 3342, T_2 \approx 6388, \dots, T_6 \approx 10539$).
3. **Bound Sector Participation:** Consequently, bound states are not negligible: at $N = 24$, the bound-state sector sums to $81,232.4$, while the scattering continuum sums to $5,419.56$. Together with the kinematic shift $\|M u\|_2^2$, they participate in an intricate, collective cancellation to reconstruct the total first-jet ratio $D_1/D_0 \approx 5.34 \times 10^5$ to 13 decimal digits (error $2.79 \times 10^{-13}$).
4. **Open Problem:** Rather than simple scattering dominance, the large-$N$ asymptotic organization of $D_1/D_0$ is governed by a collective cancellation and reorganization between the bound and scattering sectors.
