...
Tasks:
  1. Prove polynomial bound |D_1/D_0| <= C N^p via sector-decomposed resolvents.
     ...
     - Advance (Exact Even-Resolvent Operator Representation, Ground-Doublet Pole Cancellation, and Route B Operator Bound): Proved exact second-commutator identity $(Q_{\mathrm{even}} - \lambda I) K^2 c = - D_0 (K \boldsymbol\psi + M_1 d)$, established exact ground-doublet pole cancellation $K v_{\mathrm{exc}} = \|v_{\mathrm{exc}}\|^2 c + D_0 \Xi$ via the First Resolvent Identity with non-singular $\Xi \in c^\perp$, proved the exact unconditional Route B operator inequality $\rho_2^{\mathrm{exc}} \le \frac{D_0^2 \|\Xi\|^2}{M_1^{\mathrm{exc}}(1 - D_0^2 M_{2,\mathrm{exc}})}$, and proved the dual variational Rayleigh quotient representation $\rho_2^{\mathrm{exc}} = \frac{D_0^2}{\mathcal{R}_{Q_{\mathrm{odd}} - \lambda}(v_{\mathrm{exc}})}$ (Proposition 8.13). Proved that because $\|\Xi\|^2 \approx (M_1^{\mathrm{exc}})^2 \|R_{\mathrm{even}} d_{\mathrm{exc}}\|^2$ grows at most polynomially under H2 and H2$_{\mathrm{odd}}$, $\rho_2^{\mathrm{exc}}$ is exponentially quenched by the boundary factor $D_0^2 \le C_0 e^{-\sigma N}$ (H3), thereby bypassing the odd-sector tunneling ladder entirely and eliminating the independent gap hypothesis $\mathrm{H2}_{\mathrm{gap}}$.
   2. Prove polynomial (or exponentially decaying) relative tunneling-gap control H2_gap: D_0^2 / (\mu_1 - \lambda) <= C_gap N^{q_0}, replacing the independent second-moment condition in Proposition 8.10 with relative gap control.
...
| **M11** | Exact even-resolvent representation of odd coordinate residual, ground-doublet pole cancellation, and Route B operator bound | Paper 4B Proposition 8.13 | **COMPLETED** (Exact second-commutator identity $(Q_{\mathrm{even}} - \lambda I) K^2 c = - D_0 (K \boldsymbol\psi + M_1 d)$, ground-doublet pole cancellation via First Resolvent Identity, and exact master operator inequality $\rho_2^{\mathrm{exc}} \le \frac{D_0^2 \|\Xi\|^2}{M_1^{\mathrm{exc}}(1 - D_0^2 M_{2,\mathrm{exc}})}$ established unconditionally; dual variational Rayleigh quotient representation proved; eliminates reliance on excited odd tunneling gaps by transferring bound to even-sector resolvents) |
