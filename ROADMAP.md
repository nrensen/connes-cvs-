...
| **M22** | Exact Stieltjes-residue product representation for boundary weights $d_k^2$, scale-invariant bisection root audit, normalized residuals, and spectral ratio factorization in `cell76.py` | Paper 4B Proposition 8.25, Section 8.24, `cell75.py`, & `cell76.py` | **IN PROGRESS — THEOREM RIGOROUS; NUMERICAL AUDIT REPAIRED VIA CELL 76** (Proposition 8.25 proves exact unconditional Stieltjes residue formula $d_k^2 = (2N+1) \frac{\prod_{j=0}^{N-1} |E_k - z_j^*|}{\prod_{\ell \ne k} |E_k - E_\ell|}$. `cell75.out` confirmed $z_2^* \to \mu_2$ alignment contracting to $4.05 \times 10^{-3}$ and verified exact pole asymmetry $\frac{H_3}{H_2} = \alpha_2 (L_2/R_2)^2 \approx 1.5280$, but numerical residue reconstruction suffered from scale-dependent stopping ($|f(c)| \le 10^{-70}$ vs natural scale $10^{-74}$ in lowest interval $(E_0, E_1)$ at $N=24$). Deliverable `cell76.py` provides the decisive scale-invariant verification: (A) Pure bracketed bisection root finder based strictly on relative bracket width $(b-a)/(E_{j+1}-E_j) \le 10^{-55}$; (B) Scale-normalized residual $\mathrm{Res}_{\mathrm{norm}}(z_j^*) \equiv |G_d(z_j^*)| / \sum \frac{d_k^2}{|E_k - z_j^*|}$; (C) Full residue reconstruction $|d_{k,\mathrm{prod}}^2 / d_k^2 - 1| \to 0$; (D) Spectral gap ratio factorization $\alpha_j = \zeta_j \cdot (\Pi_j^{\mathrm{zeros}} \Pi_j^{\mathrm{evals}})$ and link to pole asymmetry.) |
