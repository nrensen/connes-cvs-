...
Tasks:
  1. Prove polynomial bound |D_1/D_0| <= C N^p via sector-decomposed resolvents.
     ...
     - Advance (Bound-State Ladder Decomposition, Coordinate-Energy Localization, and Operator Trace Enclosure): Proved exact operator trace identity on $H_{\mathrm{odd}}$: $\mathcal{S}_{\mathrm{coord}}(N) = D_0^2 \operatorname{Tr}_{H_{\mathrm{odd}}}[(Q_{\mathrm{odd}} - \lambda I)^{-1} P_{\perp u_0} K P_{\perp c} K] \le D_0^2 \sum_{m=1}^N m^2 [R_{\mathrm{odd},\perp}(\lambda)]_{mm}$. Decomposed $\mathcal{S}_{\mathrm{coord}} = \mathcal{S}_{\mathrm{bound}} + \mathcal{S}_{\mathrm{high}}$, proving that bound states localized beneath the barrier top carry $\mathcal{O}(1)$ coordinate kinetic energy $\sum_{j=1}^{N_{\mathrm{bound}}} \|K u_j\|^2 \le \mathcal{C}_{\mathrm{bound}} < \infty$ with $N_{\mathrm{bound}} = \mathcal{O}(1)$. Established the bound-state enclosure $\mathcal{S}_{\mathrm{bound}}(N) \le \mathcal{C}_{\mathrm{bound}} R_{\mathrm{gap}}^{\max}(N)$, completely eliminating the global polynomial factor $N^3$ from the bound-state relative-gap requirement and relaxing the continuum decoupling condition for Route B to $R_{\mathrm{gap}}^{\max}(N) \to 0$ (or bounded) (Proposition 8.17).
   2. Control the bound-state contribution to the weighted coordinate-energy measure $\mathcal{S}_{\mathrm{bound}}(N) = \sum_{j=1}^{N_{\mathrm{bound}}} \frac{D_0^2}{\mu_j - \lambda} \|P_{\perp c} K u_j\|^2$; establish whether its suppression follows from coordinate-energy localization rather than the worst-case relative gap (Milestone M12).
