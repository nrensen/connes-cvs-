...
| **M12** | Analytical control of the even-resolvent representation $\Xi$ via Stieltjes difference quotients, global Parseval operator identity, and Route B decoupling | Paper 4B Propositions 8.14 & 8.15 | **ADVANCED — EXACT GLOBAL RESOLVENT PARSEVAL ENCLOSURE ESTABLISHED** (Proved exact Stieltjes difference quotient identity $A_k = \frac{F_{\mathrm{exc}}(E_k) - F_{\mathrm{exc}}(\lambda)}{E_k - \lambda}$, smooth $k=1$ enclosure on $[\lambda, E_1]$, exact Parseval even-resolvent identity $a_j^2 \sum_{k=1}^{N-1} \frac{d_k^2}{(\mu_j - E_k)^2} = \|K u_j\|^2 - b_{0j}^2 \le N^2$, and global closed-form quadratic enclosure $\|\Xi\|^2 \le M_1^{\mathrm{exc}} \sum_{j=1}^{N-1} \frac{\|K u_j\|^2 - b_{0j}^2}{\mu_j - \lambda}$, automatically absorbing the resonant contribution and establishing $\rho_2^{\mathrm{exc}} \le \frac{N^3}{1 - D_0^2 M_{2,\mathrm{exc}}} R_{\mathrm{gap}}^{\max}(N)$ without modewise $\frac{d_k a_k}{\mu_k - E_k} = \mathcal{O}(1)$ assumptions) |
