...
Tasks:
  1. Prove polynomial bound |D_1/D_0| <= C N^p via sector-decomposed resolvents.
     ...
     - Advance (Part II Conditional): Polynomial decoupling |D_1/D_0| <= C N^p (p = \max(\eta + \gamma_e, \eta + \gamma_{\mathrm{filt}}, 2) < \infty, with \eta = \max(2, \gamma), non-sharp) established under WKB tunneling-flux (H1), even-sector transmission growth (H2), and odd-sector resolvent bound (H2_odd) hypotheses, with M_1 = O(N^{\max(2, \gamma)}) and D_0^2 M_2 = O(N^2) (Proposition 8.10).
