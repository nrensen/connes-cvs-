...
| **M22** | Exact Stieltjes-residue product formula for boundary weights $d_k^2$, interlacing zeros $z_j^*$, and odd-eigenvalue gap alignment in `cell75.py` | Paper 4B Section 8.24 & `cell75.py` | **IN PROGRESS** (Audit the exact Stieltjes residue formula $d_k^2 = (2N+1) \frac{\prod |E_k - z_j^*|}{\prod |E_k - E_l|}$ where $z_j^*$ are the exact zeros of $G_d(z) = 0$ on $(E_j, E_{j+1})$; measure alignment with odd eigenvalues $|z_j^* - \mu_j| / (E_{j+1}-E_j)$; audit the odd-spectrum surrogate product formula $d_k^{2, \mathrm{odd}}$ and discrepancy ratios across $N \in \{8, 12, 16, 20, 24\}$ at 50 dps) |
