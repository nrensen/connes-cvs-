...
| **M12** | Analytical control of the even-resolvent representation $\Xi$ and Route B decoupling | Paper 4B Section 8.13 | **PLANNED** (Establish polynomial bound $\|\Xi\|^2 \le C_\Xi N^{r_\Xi}$ on the unified mode vector $\Xi = \sum_{k \ge 1} d_k [\sum_{j \ge 1} \frac{a_j^2}{(\mu_j - \lambda)(\mu_j - E_k)}] u_k$ to prove $\rho_2^{\mathrm{exc}} \le C e^{-\sigma N} N^p \to 0$ without excited odd gap hypotheses) |
