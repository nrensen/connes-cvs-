...
| **M28** | Finite-Core + Tail Architecture, Cumulative Lower Spectral Mass, and Analytical Tail Suppression in `cell82.py` | Paper 4B Section 8.25 & `cell82.py` | **IN PROGRESS** (Audit core-tail partition across thresholds $L \in \{4, 6, 8\}$; evaluate direct displacement tail $\mathcal{T}_{2, \mathrm{tail}} = \sum_{\ell > L} \frac{\Delta_2 \delta_\ell}{|E_2 - z_\ell^*||E_3 - E_\ell|}$; audit cumulative lower spectral mass $M(k) = \sum_{m \le k} d_m^2$ and bounds on denominator $S_\ell = P_\ell - N_\ell$; test uniform tail summability across $N \in \{8, 12, 16, 20, 24\}$ at 70 dps) |
