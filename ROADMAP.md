...
Tasks:
  1. Prove polynomial bound |D_1/D_0| <= C N^p via sector-decomposed resolvents.
     ...
     - Advance (Exact Dipole Factorization of the First Relative Tunneling Gap and Even Resolvent Cancellation): Proved the exact finite-$N$ algebraic factorization $R_{\mathrm{gap}}(N) \equiv \frac{D_0^2}{\mu_1 - \lambda} = \frac{b_{01}^2}{\mathcal{R}_1}$ with $\mathcal{R}_1 \equiv \frac{a_1^2}{\mu_1 - \lambda}$ strictly $\mathcal{O}(1)$ unconditionally (Proposition 8.19). Established coordinate wavepacket upper enclosure $b_{01}^2 \le \|P_{\perp u_0} Kc\|^2 = \|v_{\mathrm{exc}}\|^2 \implies R_{\mathrm{gap}}(N) \le \frac{\|v_{\mathrm{exc}}\|^2}{\mathcal{R}_1}$, and proved the exact even-resolvent cancellation identity $b_{01}^2 = \|Ku_1\|^2 - a_1^2 \sum_{k=1}^N \frac{d_k^2}{(\mu_1 - E_k)^2}$, reducing the relative tunneling gap suppression to an explicit cancellation between the coordinate derivative norm $\|Ku_1\|^2$ and the excited even-resolvent projection.
...
| **M14** | Exact reduction and asymptotic control of the first relative tunneling gap $R_{\mathrm{gap}} = \frac{D_0^2}{\mu_1 - \lambda}$ | Paper 4B Proposition 8.19 & `cell69.py` | **ADVANCED — EXACT FACTORIZATION ESTABLISHED** (Proved exact dipole factorization $R_{\mathrm{gap}} = b_{01}^2 / \mathcal{R}_1$ and wavepacket enclosure $R_{\mathrm{gap}} \le \|v_{\mathrm{exc}}\|^2 / \mathcal{R}_1$ with $\mathcal{R}_1 \approx 5.13 = \mathcal{O}(1)$; proved even-resolvent cancellation identity $b_{01}^2 = \|Ku_1\|^2 - a_1^2 \sum_{k=1}^N \frac{d_k^2}{(\mu_1 - E_k)^2}$; audited identities in `cell69.py` across $N \in \{8, 12, 16, 20, 24\}$, showing mode 1 carries $> 99.999\%$ of $\|v_{\mathrm{exc}}\|^2$) |
