...
| **M27** | Tri-partite spectral audit (low/bulk/edge), weighted remote sum $S_j(N)$ vs $\log \Pi_j$, 4-factor balance & scaling in `cell81.py` | Paper 4B Section 8.25 & `cell81.py` | **IN PROGRESS** (Audit weighted remote sum $S_2(N)$ against $\sum(\omega_{2,\ell}-1)$ and $\log \Pi_{2,\mathrm{remote}}$; evaluate three-zone decomposition $S_2 = S_{\mathrm{low}} + S_{\mathrm{bulk}} + S_{\mathrm{edge}}$; audit four competing factors $\alpha_\ell, D_{2,\ell}^{-2}, (1-\varepsilon_\ell)^{-1}, \Delta_2\Delta_\ell$; audit asymptotic scaling and uniform boundedness of $S_2(N)$ across $N \in \{8, 12, 16, 20, 24\}$ at 70 dps) |
