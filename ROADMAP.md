...
| **M-G1.5** | Gate 1 | **Subspace Compression & Certified Coupled Cancellation:** Exact prime decomposition $Q_{\mathrm{prime}}^{\mathrm{even}} \equiv -\widetilde{W} + \widetilde{\mathcal{D}}^{\mathrm{per}} + \Delta\widetilde{\mathcal{D}}$ and Two-Regime Multiplier Theorem ($\inf_{m \ge 1} \Omega(m) \ge 0.1567$) certified; Cell 131/132 50-dps audits proved that competition operator $\widehat{\mathcal{Q}}_{\mathrm{comp}} \equiv \widehat{\Omega} - \widehat{\mathcal{K}}_{\mathrm{neg}}$ develops a strictly solitary negative direction ($k_{\mathrm{neg}} \equiv 1$, $\mu_0 \approx -0.487$, $\mu_1 \ge +0.348 > 0$); exact Archimedean ($+0.440$) and pole ($+0.212$) terms decisively restore positivity ($\rho_{\mathrm{restore}} \approx 1.339 > 1$, $R_{\mathrm{net}} \approx +0.165 > 0$), with residual $\|\mathcal{R}\|_{\mathrm{op}} \le 1.03 \times 10^{-48}$ and $|\Delta_{\mathrm{Rayleigh}}| \le 5.61 \times 10^{-50}$; net energy stabilizes near $0.165$ with frozen peak mode $m^* = 26$ | [`cell130.md`](file:///c:/data/github/connes-cvs-/cell130.md) / [`cell131.md`](file:///c:/data/github/connes-cvs-/cell131.md) / [`cell132.md`](file:///c:/data/github/connes-cvs-/cell132.md) | **AUDITED & CERTIFIED (Cell 132)** |
| **M-G1.6** | Gate 1 | **Unified Representation & Variational Formulation:** Coordinate-space mapping $T_{v_{\mathrm{bad}}}(t)$ and exact Euler–Lagrange constrained variational structure $(\mathcal{Q}_{\mathrm{comp}} - \mu_0 I) v_{\mathrm{bad}} = \sum \lambda_k u_k$; Bridge A closed to machine precision ($< 1.18 \times 10^{-49}$) via exact closed-form Fourier integral; gauge-invariant force field $F_{\mathrm{constr}}(t)$; stable three-way partition $R_{\mathrm{mult}} - R_W + R_{\mathcal{D}}^{\mathrm{true}} = -0.487$; continuum quadratic form $\mathcal{E}[T]$ on $H^{\log}([0, L])$ and variational lower bound $\mu_0 > -1/2$ | [`cell133.md`](file:///c:/data/github/connes-cvs-/cell133.md) / [`cell134.md`](file:///c:/data/github/connes-cvs-/cell134.md) / [`cell135.md`](file:///c:/data/github/connes-cvs-/cell135.md) / [`cell136.md`](file:///c:/data/github/connes-cvs-/cell136.md) | **IN PROGRESS (Cells 133–135 Certified; Cell 136 Authored & Ready)** |
