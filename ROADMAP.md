...
| **M19** | Targeted semiclassical action competition $\tau_2 > \gamma_2$, even-resolvent pole cancellation $\mathcal{Q}_2 = \frac{D_0^2 + (\mu_1-\lambda)^2 \mathcal{E}_{\mathrm{even}, 1}}{D_0^2 + (\mu_2-\lambda)^2 \mathcal{E}_{\mathrm{even}, 2}}$, and authoring of `cell72.py` | Paper 4B Proposition 8.23 & `cell72.py` | **IN PROGRESS** (Formulated exact even-resolvent pole cancellation $\mathcal{Q}_j = \frac{D_0^2 + (\mu_1-\lambda)^2 \mathcal{E}_{\mathrm{even}}(\mu_1)}{D_0^2 + (\mu_j-\lambda)^2 \mathcal{E}_{\mathrm{even}}(\mu_j)}$ and action-competition condition $\tau_2 > \gamma_2$; identified $j=2$ as governing $99.997\%$ of the excited wavepacket tail; authoring `cell72.py` to audit logarithmic scaling rates $\gamma_2(N), \tau_2(N), \sigma_{T_2}(N)$ across $N \in \{8, \dots, 24\}$) |
