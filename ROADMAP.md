...
| **M12** | Analytical control of the even-resolvent representation $\Xi$ via Stieltjes difference quotients and Route B decoupling | Paper 4B Proposition 8.14 | **PLANNED** (Establish polynomial bound $\|\Xi\|^2 = \sum_{k \ge 1} d_k^2 [\frac{F_{\mathrm{exc}}(E_k) - F_{\mathrm{exc}}(\lambda)}{E_k - \lambda}]^2 \le C_\Xi N^{r_\Xi}$ using smooth ground enclosure on $[\lambda, E_1]$, high-mode spectral filtering enclosure $A_k^{(> k)} \le \frac{M_1^{\mathrm{exc}}}{\mu_{k+1}-E_k}$, and parity-doublet transmission matching to prove $\rho_2^{\mathrm{exc}} \le C e^{-\sigma N} N^p \to 0$ without excited odd gap hypotheses) |
