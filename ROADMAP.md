...
Tasks:
  1. Prove polynomial bound |D_1/D_0| <= C N^p via sector-decomposed resolvents.
     ...
     - Advance (Global Even-Resolvent Operator Representation, Parseval Identity, and Resonant Absorption): Proved exact global operator identity $\Xi = g(Q_{\mathrm{even}}) d_{\mathrm{exc}} = \int \frac{d\nu_{\mathrm{exc}}(\mu)}{\mu - \lambda} (\mu I - Q_{\mathrm{even}})^{-1} d_{\mathrm{exc}} = M_1^{\mathrm{exc}} \sum_{j \ge 1} w_j (\mu_j I - Q_{\mathrm{even}})^{-1} d_{\mathrm{exc}}$. Established exact finite-$N$ Parseval even-resolvent identity $a_j^2 \sum_{k=1}^{N-1} \frac{d_k^2}{(\mu_j - E_k)^2} = \|K u_j\|^2 - b_{0j}^2 \le N^2$ for every excited odd mode without eigenvalue denominators (with $a_j \ne 0$ proved by strict spectral interlacing). Proved second-generation coordinate-derivative wavepacket geometry $\Xi = \frac{1}{D_0} P_{\perp c} K v_{\mathrm{exc}} \implies \|\Xi\|^2 = \frac{1}{D_0^2} \|P_{\perp c} K v_{\mathrm{exc}}\|^2$. Established global Stieltjes divided-difference kernel representation $\|\Xi\|^2 = \iint [\frac{G_{\mathrm{even}}(\mu') - G_{\mathrm{even}}(\mu)}{\mu - \mu'}] \frac{d\nu_{\mathrm{exc}}(\mu)}{\mu - \lambda} \frac{d\nu_{\mathrm{exc}}(\mu')}{\mu' - \lambda}$ and closed-form upper bound $\|\Xi\|^2 \le M_1^{\mathrm{exc}} \sum_{j \ge 1} \frac{\|K u_j\|^2 - b_{0j}^2}{\mu_j - \lambda}$, automatically absorbing the resonant contribution and reducing the second-moment bound directly to $\rho_2^{\mathrm{exc}} \le \frac{N^3}{1 - D_0^2 M_{2,\mathrm{exc}}} R_{\mathrm{gap}}^{\max}(N)$ without modewise $\mathcal{O}(1)$ assumptions (Proposition 8.15).
      ...
      - Advance (Exact Variational Energy Excess of Coordinate Wavepacket $K c$, Rayleigh Quotient Gap Enclosure, and Asymptotic Alignment Sandwich): Proved the exact finite-$N$ energy expectation $\langle Kc, Q_{\mathrm{odd}} Kc \rangle = \lambda \|Kc\|^2 + D_0^2 M_1$ from the rank-two commutator $[K, Q] = \boldsymbol\psi d^T - d \boldsymbol\psi^T$ unconditionally (Proposition 8.21). Evaluated the energy excess above the odd ground state $E_{\mathrm{exc}} \equiv \langle Kc, (Q_{\mathrm{odd}} - \mu_0 I) Kc \rangle = D_0^2 M_1^{\mathrm{exc}} - (\mu_0 - \lambda) \|P_{\perp u_0} Kc\|^2$ with exact ground-doublet pole cancellation. Proved the sharp variational Rayleigh quotient spectral-gap enclosure $\|P_{\perp u_0} Kc\|^2 \le \frac{D_0^2 M_1^{\mathrm{exc}}}{\mu_1 - \lambda} = M_1^{\mathrm{exc}} R_{\mathrm{gap}}(N)$, establishing the exact two-sided alignment sandwich $\frac{\mathcal{R}_1}{\|Kc\|^2} R_{\mathrm{gap}}(N) \le \frac{\|P_{\perp u_0} Kc\|^2}{\|Kc\|^2} \le \frac{M_1^{\mathrm{exc}}}{\|Kc\|^2} R_{\mathrm{gap}}(N)$ that clamps the normalized wavepacket misalignment $\frac{\|P_{\perp u_0} Kc\|^2}{\|Kc\|^2} \in [1.31 \times 10^{-6}, 2.38 \times 10^{-5}]$ at $N=24$ (saturating the lower bound within $0.0005\%$). Identified the fundamental spectral moment obstruction: $E_{\mathrm{exc}} = \int (\mu - \mu_0) d\nu_v$ is the first moment of the excited wavepacket spectral measure $d\nu_v = \sum_{j \ge 1} b_{0j}^2 \delta_{\mu_j}$, whereas the required misalignment $\|P_{\perp u_0} Kc\|^2 = D_0^2 M_{2,\mathrm{exc}} = \int d\nu_v$ is the zeroth moment; because the spectral gap $\mu_1 - \mu_0 \sim e^{-\sigma_1 N}$ collapses exponentially, the smallness of $E_{\mathrm{exc}} \le D_0^2 M_1^{\mathrm{exc}} \to 0$ does not by itself control the zeroth moment. Reoriented the active continuum agenda to direct zeroth-moment control of the excited coordinate wavepacket ($D_0^2 M_{2,\mathrm{exc}} \to 0$) without dividing by the closing spectral gap.
      - Advance (First-Mode Spectral Concentration, Relative Tail Ratio, and Elimination of $D_0^2$): Proved that the relative excited tail ratio $\varepsilon_N \equiv \frac{\sum_{j \ge 2} b_{0j}^2}{b_{01}^2} = \sum_{j=2}^{N-1} \frac{a_j^2}{a_1^2} (\frac{\mu_1 - \lambda}{\mu_j - \lambda})^2$ eliminates the dangerous global boundary-layer factor $D_0^2$ identically unconditionally (Proposition 8.22). Formulated the quantitative finite-$N$ estimate $|\int f \, d\bar\nu_N - f(\mu_1^{(N)})| \le 3 \|f\|_\infty \varepsilon_N$ characterizing asymptotic concentration of the normalized excited spectral measure onto the moving Dirac mass $\delta_{\mu_1^{(N)}}$. Proved the exact asymptotic residual equivalence $\|P_{\perp u_0} Kc\|^2 = D_0^2 M_{2,\mathrm{exc}} = b_{01}^2 (1 + \varepsilon_N) \sim b_{01}^2 = \mathcal{R}_1 R_{\mathrm{gap}}(N)$, reducing the zeroth-moment control problem to relative spectral concentration. Proved unconditional high-energy enclosure $\varepsilon_N^{\mathrm{high}} \le \frac{\mu_1 - \lambda}{c_{\mathrm{high}}^2 \mathcal{R}_1(N)} \|P_{\mathrm{high}} d\|^2$, yielding exponential decoupling $\varepsilon_N^{\mathrm{high}} = \mathcal{O}(e^{-\sigma_1 N} N) \to 0$ conditionally upon $\inf_N \mathcal{R}_1(N) > 0$ and $\mu_1 - \lambda = \mathcal{O}(e^{-\sigma_1 N})$, while isolating the remaining open control target strictly to the bound-state ladder.
