...
| **M17** | First-mode spectral concentration of coordinate wavepacket, relative tail ratio $\varepsilon_N = \sum_{j \ge 2} \frac{a_j^2}{a_1^2} (\frac{\mu_1 - \lambda}{\mu_j - \lambda})^2$, and mode-by-mode spectral filtering | Paper 4B Proposition 8.22 & `cell70.py` | **IN PROGRESS** (Exact finite-$N$ tail ratio $\varepsilon_N$ and $D_0^2$ elimination proved; quantitative finite-$N$ concentration estimate on moving Dirac mass $\delta_{\mu_1^{(N)}}$ established; residual equivalence $\|P_{\perp u_0} Kc\|^2 = b_{01}^2(1+\varepsilon_N)$ proved; conditional high-energy decoupling $\varepsilon_N^{\mathrm{high}} = \mathcal{O}(e^{-\sigma_1 N} N) \to 0$ formulated; `cell70.py` authored to audit mode-by-mode tail ratios and spectral filtering across $N \in \{8, \dots, 24\}$) |
| **M18** | Exact Stieltjes derivative factorization $\frac{a_j^2}{a_1^2} = \frac{\|K u_j\|^2}{\|K u_1\|^2} \frac{G_d'(\mu_1)}{G_d'(\mu_j)}$ and three-factor bound-state tail filtering $T_j = \mathcal{K}_j \cdot \mathcal{G}_j \cdot \mathcal{S}_j$ | Paper 4B Proposition 8.23 & `cell71.py` | **IN PROGRESS** (Exact finite-$N$ Stieltjes derivative representation $a_j^2 = \|K u_j\|^2 / G_d'(\mu_j)$ and overlap ratio factorization proved unconditionally; three-factor decomposition $T_j = \mathcal{K}_j \cdot \mathcal{G}_j \cdot \mathcal{S}_j$ established; polynomial overlap sufficiency proved; `cell71.py` authored to audit $\mathcal{K}_j, \mathcal{G}_j, \mathcal{S}_j$ across $N \in \{8, \dots, 24\}$) |
