...
Tasks:
  1. Prove polynomial bound |D_1/D_0| <= C N^p via sector-decomposed resolvents.
     ...
     - Advance (Part II Conditional): Polynomial decoupling |D_1/D_0| <= C N^p (p = 1 + \max(2, 1+\gamma) + \max(\gamma_e, \gamma_o) < \infty, non-sharp) established under WKB tunneling-flux (H1) and uniform polynomial spectral-gap (H2) hypotheses, with M_1 = O(N^{\max(2, 1+\gamma)}) and D_0^2 M_2 = O(N^2) (Proposition 8.10).
