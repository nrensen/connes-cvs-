...
| **M26** | Spectral-wide audit of the Stieltjes sign ratio $\varepsilon_\ell = N_\ell / P_\ell$, upper-edge boundary weights, and boundary-weight ladder geometric decay in `cell80.py` | Paper 4B Section 8.25 & `cell80.py` | **IN PROGRESS** (Sweep $\varepsilon_\ell$ across all $\ell \in \{1, \dots, N-1\}$ for $N \in \{8, 12, 16, 20, 24\}$; audit edge-mode forensics at $\ell = N-1$; test restored validity of corrected closed bound $\omega_{2, \ell} - 1 < \frac{\Delta_j \Delta_\ell}{\alpha_\ell (1 - \varepsilon_\ell) D_{2, \ell}^2}$; test geometric decay in boundary-weight ladder $d_k^2 / d_\ell^2 \le C q^{\ell - k}$ at 70 dps) |
