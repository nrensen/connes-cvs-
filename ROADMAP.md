...
Tasks:
  1. Prove polynomial bound |D_1/D_0| <= C N^p via sector-decomposed resolvents.
     ...
     - Advance (Global Even-Resolvent Operator Representation, Parseval Identity, and Resonant Absorption): Proved exact global operator identity $\Xi = g(Q_{\mathrm{even}}) d_{\mathrm{exc}} = \int \frac{d\nu_{\mathrm{exc}}(\mu)}{\mu - \lambda} (\mu I - Q_{\mathrm{even}})^{-1} d_{\mathrm{exc}} = M_1^{\mathrm{exc}} \sum_{j \ge 1} w_j (\mu_j I - Q_{\mathrm{even}})^{-1} d_{\mathrm{exc}}$. Established exact finite-$N$ Parseval even-resolvent identity $a_j^2 \sum_{k=1}^{N-1} \frac{d_k^2}{(\mu_j - E_k)^2} = \|K u_j\|^2 - b_{0j}^2 \le N^2$ for every excited odd mode without eigenvalue denominators (with $a_j \ne 0$ proved by strict spectral interlacing). Proved second-generation coordinate-derivative wavepacket geometry $\Xi = \frac{1}{D_0} P_{\perp c} K v_{\mathrm{exc}} \implies \|\Xi\|^2 = \frac{1}{D_0^2} \|P_{\perp c} K v_{\mathrm{exc}}\|^2$. Established global Stieltjes divided-difference kernel representation $\|\Xi\|^2 = \iint [\frac{G_{\mathrm{even}}(\mu') - G_{\mathrm{even}}(\mu)}{\mu - \mu'}] \frac{d\nu_{\mathrm{exc}}(\mu)}{\mu - \lambda} \frac{d\nu_{\mathrm{exc}}(\mu')}{\mu' - \lambda}$ and closed-form upper bound $\|\Xi\|^2 \le M_1^{\mathrm{exc}} \sum_{j \ge 1} \frac{\|K u_j\|^2 - b_{0j}^2}{\mu_j - \lambda}$, automatically absorbing the resonant contribution and reducing the second-moment bound directly to $\rho_2^{\mathrm{exc}} \le \frac{N^3}{1 - D_0^2 M_{2,\mathrm{exc}}} R_{\mathrm{gap}}^{\max}(N)$ without modewise $\mathcal{O}(1)$ assumptions (Proposition 8.15).
      ...
      - Advance (First-Mode Spectral Concentration, Relative Tail Ratio, and Elimination of $D_0^2$): Proved that the relative excited tail ratio $\varepsilon_N \equiv \frac{\sum_{j \ge 2} b_{0j}^2}{b_{01}^2} = \sum_{j=2}^{N-1} \frac{a_j^2}{a_1^2} (\frac{\mu_1 - \lambda}{\mu_j - \lambda})^2$ eliminates the dangerous global boundary-layer factor $D_0^2$ identically unconditionally (Proposition 8.22). Formulated the weak concentration of the normalized excited spectral measure $\frac{d\nu_v}{\|v_{\mathrm{exc}}\|^2} \rightharpoonup \delta_{\mu_1}$. Proved the exact asymptotic residual equivalence $\|P_{\perp u_0} Kc\|^2 = D_0^2 M_{2,\mathrm{exc}} = b_{01}^2 (1 + \varepsilon_N) \sim b_{01}^2 = \mathcal{R}_1 R_{\mathrm{gap}}(N)$, reducing the zeroth-moment control problem to relative spectral concentration. Proved exponential high-energy decoupling $\varepsilon_N^{\mathrm{high}} = \mathcal{O}(e^{-\sigma_1 N} N) \to 0$, isolating the active target to the bound-state sector where $(\frac{\mu_1 - \lambda}{\mu_j - \lambda})^2$ provides automatic spectral suppression.
...
| **M17** | First-mode spectral concentration of coordinate wavepacket, relative tail ratio $\varepsilon_N = \sum_{j \ge 2} \frac{a_j^2}{a_1^2} (\frac{\mu_1 - \lambda}{\mu_j - \lambda})^2$, and mode-by-mode spectral filtering | Paper 4B Proposition 8.22 & `cell70.py` | **IN PROGRESS** (Exact finite-$N$ tail ratio $\varepsilon_N$ and $D_0^2$ elimination proved; measure concentration $\frac{d\nu_v}{\|v_{\mathrm{exc}}\|^2} \rightharpoonup \delta_{\mu_1}$ formulated; residual equivalence $\|P_{\perp u_0} Kc\|^2 = b_{01}^2(1+\varepsilon_N)$ established; `cell70.py` authored to audit mode-by-mode tail ratios and spectral filtering across $N \in \{8, \dots, 24\}$) |
