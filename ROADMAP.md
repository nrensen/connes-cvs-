...
Tasks:
  1. Prove polynomial bound |D_1/D_0| <= C N^p via sector-decomposed resolvents.
     ...
     - Advance (Global Even-Resolvent Operator Representation, Parseval Identity, and Resonant Absorption): Proved exact global operator identity $\Xi = g(Q_{\mathrm{even}}) d_{\mathrm{exc}} = \int \frac{d\nu_{\mathrm{exc}}(\mu)}{\mu - \lambda} (\mu I - Q_{\mathrm{even}})^{-1} d_{\mathrm{exc}} = M_1^{\mathrm{exc}} \sum_{j \ge 1} w_j (\mu_j I - Q_{\mathrm{even}})^{-1} d_{\mathrm{exc}}$. Established exact finite-$N$ Parseval even-resolvent identity $a_j^2 \sum_{k=1}^{N-1} \frac{d_k^2}{(\mu_j - E_k)^2} = \|K u_j\|^2 - b_{0j}^2 \le N^2$ for every excited odd mode without eigenvalue denominators (with $a_j \ne 0$ proved by strict spectral interlacing). Proved second-generation coordinate-derivative wavepacket geometry $\Xi = \frac{1}{D_0} P_{\perp c} K v_{\mathrm{exc}} \implies \|\Xi\|^2 = \frac{1}{D_0^2} \|P_{\perp c} K v_{\mathrm{exc}}\|^2$. Established global Stieltjes divided-difference kernel representation $\|\Xi\|^2 = \iint [\frac{G_{\mathrm{even}}(\mu') - G_{\mathrm{even}}(\mu)}{\mu - \mu'}] \frac{d\nu_{\mathrm{exc}}(\mu)}{\mu - \lambda} \frac{d\nu_{\mathrm{exc}}(\mu')}{\mu' - \lambda}$ and closed-form upper bound $\|\Xi\|^2 \le M_1^{\mathrm{exc}} \sum_{j \ge 1} \frac{\|K u_j\|^2 - b_{0j}^2}{\mu_j - \lambda}$, automatically absorbing the resonant contribution and reducing the second-moment bound directly to $\rho_2^{\mathrm{exc}} \le \frac{N^3}{1 - D_0^2 M_{2,\mathrm{exc}}} R_{\mathrm{gap}}^{\max}(N)$ without modewise $\mathcal{O}(1)$ assumptions (Proposition 8.15).
      ...
      - Advance (Positive Regularized Stieltjes Function $H(\mu)$, Exact Modal Ratio $\mathcal{Q}_j = H(\mu_1)/H(\mu_j)$, and Interlacing Pole Lower Bound): Defined the manifestly positive rational function $H(\mu) \equiv (\mu - \lambda)^2 G_d'(\mu) = D_0^2 + \sum_{k=1}^N d_k^2 (\frac{\mu - \lambda}{E_k - \mu})^2$, unifying overlap growth and spectral gap suppression into the single exact ratio $\mathcal{Q}_j = \frac{H(\mu_1)}{H(\mu_j)}$ and $T_j = \mathcal{K}_j \frac{H(\mu_1)}{H(\mu_j)}$ (Proposition 8.24). This eliminates both intermediate scales ($\mathcal{G}_2 \sim 10^5$ and $\mathcal{S}_2 \sim 10^{-11}$) in favor of the growth of $H(\mu)$ between $\mu_1$ and $\mu_j$. Proved the unconditional nearest-pole lower bound $H(\mu_j) \ge d_{k_{\mathrm{near}}}^2 (\frac{\mu_j - \lambda}{E_{k_{\mathrm{near}}} - \mu_j})^2$ and formulated Milestone M20 to audit the term-by-term pole distribution across $k \in \{0, \dots, N\}$ in `cell73.py`.
...
| **M20** | Positive regularized Stieltjes function $H(\mu) = (\mu-\lambda)^2 G_d'(\mu)$, exact modal ratio $\mathcal{Q}_j = H(\mu_1)/H(\mu_j)$, and interlacing pole audit in `cell73.py` | Paper 4B Proposition 8.24 & `cell73.py` | **IN PROGRESS** (Formulated positive regularized Stieltjes function $H(\mu)$ and exact modal ratio $\mathcal{Q}_j = H(\mu_1)/H(\mu_j)$, eliminating intermediate scales $\mathcal{G}_2 \sim 10^5$ and $\mathcal{S}_2 \sim 10^{-11}$; established nearest-pole lower bound $H(\mu_j) \ge d_{k_{\mathrm{near}}}^2 (\frac{\mu_j-\lambda}{E_{k_{\mathrm{near}}}-\mu_j})^2$; authoring `cell73.py` to audit term-by-term pole distribution, identify dominant poles, and verify nearest-pole lower bounds across $N \in \{8, 12, 16, 20, 24\}$) |
