...
| **M24** | Exact pairwise mode cancellation identity $\omega_{j, \ell} - 1$, exact Stieltjes displacement formula $\delta_\ell = \frac{d_\ell^2}{\sum_{k \ne \ell} \frac{d_k^2}{E_k - z_\ell^*}}$, and quantitative remote product bounds in `cell78.py` | Paper 4B Proposition 8.26, Section 8.25, & `cell78.py` | **IN PROGRESS** (Audit the exact finite-$N$ algebraic identity $\omega_{j, \ell} - 1 = \frac{\Delta_j \delta_\ell}{|E_j - z_\ell^*| |E_{j+1} - E_\ell|}$ down to precision floor across all remote modes $\ell \notin \{j, j+1\}$; audit the exact Stieltjes displacement formula $\delta_\ell = \frac{d_\ell^2}{\sum_{k \ne \ell} \frac{d_k^2}{E_k - z_\ell^*}}$ against direct bisection roots; evaluate the quadratic decay bound $|\omega_{j, \ell}-1| \le \frac{\Delta_j \delta_\ell}{D_{j, \ell}^2}$; analyze the convergence rate of the remote tail product $\prod_{|\ell - j| \ge 2} \omega_{j, \ell} \to 1$ across $N \in \{8, 12, 16, 20, 24\}$ at 70 dps) |
